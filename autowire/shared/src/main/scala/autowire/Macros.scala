...
object Macros {
  ...
  case class Luz[T](s: String) extends Check[T] {
    def map[V](f: T => V): Luz[V] = Luz[V](s)
    def flatMap[V](f: T => Check[V]): Luz[V] = Luz[V](s)
    def withFilter(f: T => Boolean): Luz[T] = Luz[T](s)
  }

  case class Win[T](t: T, s: String) extends Check[T] {
    def map[V](f: T => V): Win[V] = Win(f(t), s)
    def flatMap[V](f: T => Check[V]): Check[V] = f(t)
    def withFilter(f: T => Boolean): Check[T] with Product = if (f(t)) this else Luz(s)
  }
  ...
  : c.Expr[Future[Result]] = {
    ...
    val res = for {
      ...
      (unwrapTree: Tree, methodName: TermName, args: Seq[Tree], prelude: Seq[Tree], deadNames: Seq[String]@unchecked) = (contents: Tree) match {
        ...
        case x => c.abort(x.pos, s"You can't call the .call() method on $x, only on autowired function calls.")
      }
      ...
      (oTree, memPath) = {
        ...
        def getMempath(tree: Tree, path: List[String]): (Tree, List[String]) = {
          tree match {
            case Select(t, n) => getMempath(t, n.toTermName.toString :: path)
            case _            => tree -> path
          }
        }
    ...
    res match {
      case Win(tree, _) => c.Expr[Future[Result]](tree)
  ...
  : c.Expr[Router[PickleType]] = {
    ...
    val routes   = help.getAllRoutesForClass(pt, target, topClass, topClass.typeSymbol.fullName.split('.').toSeq, Nil).toList
