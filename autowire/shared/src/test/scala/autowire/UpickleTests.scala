...
object UpickleTests extends TestSuite {
  ...
  object Bundle extends GenericClientServerBundle[String, upickle.default.Reader, upickle.default.Writer] {
    ...
    def write[T: upickle.default.Writer](t: T) = {
      upickle.default.write(t)
    }

    def read[T: upickle.default.Reader](t: String) = {
      upickle.default.read[T](t)
    }
      ...
      test("inputError") {
        ...
        test("keysInvalid") - {
          test - {
            check(Map("x" -> "[]", "ys" -> "234")) {
              case Error.InvalidInput(
              Error.Param.Invalid("x", upickle.core.AbortException(_, _, _, _, _)),
              Error.Param.Invalid("ys", upickle.core.AbortException(_, _, _, _, _))
              )
              =>
            }
          }
        ...
        test("invalidJson") - {
          test - {
            check(Map("x" -> "]", "ys" -> "2}34")) {
              case Error.InvalidInput(
              Error.Param.Invalid("x", ujson.ParseException(_, _, _, _)),
              Error.Param.Invalid("ys", upickle.core.AbortException(_, _, _, _, _))
              ) =>
            }
          }
